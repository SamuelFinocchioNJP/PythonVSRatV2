<?php 
    ...
    // Request received 
        file_put_contents(RESPONSE_STORAGE, mb_convert_encoding(file_get_contents('php://input'), "UTF-8"));
        file_put_contents(LOG, mb_convert_encoding(file_get_contents('php://input'), "UTF-8") . PHP_EOL, FILE_APPEND);
    } else {
        // No request received
        if(file_exists(RESPONSE_STORAGE)) {
            // db not empty
            echo (file_get_contents(RESPONSE_STORAGE));
        } else {
            echo ("Can not find " . RESPONSE_STORAGE);
        }
    } 
?>